...
def test_init_registers_projects_and_content_references() -> None:
    ...
    assert set(registry.project_references[ContentType.MATERIALS]) == {
...
def test_add_project_registers_project_and_content_references() -> None:
    ...
    assert registry.project_references[ContentType.MATERIALS] == (
...
def test_content_reference_operations_update_registry() -> None:
    ...
    registry.add_content_reference(first_reference)
    registry.add_content_reference(second_reference)

    assert registry.project_references[ContentType.MATERIALS] == (first_reference, second_reference)

    registry.remove_content_reference(first_reference)

    assert registry.project_references[ContentType.MATERIALS] == (second_reference,)
